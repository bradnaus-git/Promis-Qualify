...
import { Search, Mail, Phone, ExternalLink, Award, Briefcase, Filter, X, ChevronRight, UserCheck, Linkedin } from "lucide-react";
                  ...
                  <div className="flex items-center gap-1.5">
                    {emp.email && (
                      <a
                        ...
                        className="p-1.5 rounded-md text-slate-400 hover:text-[#009FE3] hover:bg-slate-100 transition-colors"
                        title={emp.email}
                    ...
                    {emp.linkedIn && (
                      <a
                        ...
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-[#0A66C2]/10 hover:bg-[#0A66C2] text-[#0A66C2] hover:text-white transition-all text-[11px] font-semibold group/li"
                        title={`LinkedIn: ${emp.name}`}
                        aria-label={`LinkedIn profil for ${emp.name}`}
                      >
                        <Linkedin className="w-3.5 h-3.5" />
                        <span className="font-semibold text-[10px]">in</span>
                      </a>
                    )}
                  </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2.5">
                {selectedEmployee.email && (
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-medium text-slate-700 hover:text-slate-900 transition-colors"
                  >
                    <Mail className="w-3.5 h-3.5 text-[#009FE3]" />
                    <span className="truncate max-w-[150px] sm:max-w-none">{selectedEmployee.email}</span>
                  </a>
                )}
                ...
                {selectedEmployee.linkedIn && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0A66C2] hover:bg-[#084e96] text-white text-xs font-semibold transition-all shadow-sm"
                    title={`LinkedIn: ${selectedEmployee.name}`}
                  >
                    <Linkedin className="w-3.5 h-3.5" />
                    <span>{lang === "no" ? "Se LinkedIn-profil" : "View LinkedIn Profile"}</span>
              ...
              <a
                href="#contact"
                onClick={() => setSelectedEmployee(null)}
                className="px-4 py-2 rounded-lg bg-[#009FE3] hover:bg-[#0088C5] text-white text-xs font-semibold text-center transition-all shadow-sm shrink-0"
              >
